...
void DRV832x::print_faults(void)
{
    ...
    if(val1 & (1<<9)){printf("VDS_OCP\n\r");}     //Indicates VDS monitor overcurrent fault condition
    if(val1 & (1<<8)){printf("GDF\n\r");}         //Indicates gate drive fault condition
    if(val1 & (1<<7)){printf("UVLO\n\r");}        //Indicates undervoltage lockout fault condition
    if(val1 & (1<<6)){printf("OTSD\n\r");}        //Indicates overtemperature shutdown
    if(val1 & (1<<5)){printf("VDS_HA\n\r");}      //Indicates VDS overcurrent fault on the A high-side MOSFET
    ...
    if(val2 & (1<<10)){printf("SA_OC\n\r");}      //Indicates overcurrent on phase A sense amplifier 
    if(val2 & (1<<9)){printf("SB_OC\n\r");}
    if(val2 & (1<<8)){printf("SC_OC\n\r");}
    if(val2 & (1<<7)){printf("OTW\n\r");}         //Indicates overtemperature warning
    if(val2 & (1<<6)){printf("CPUV\n\r");}        //Indicates charge pump undervoltage fault condition
    if(val2 & (1<<5)){printf("VGS_HA\n\r");}      //Indicates gate drive fault on the A high-side MOSFET
