...
int main() {
    ...
    */
    ...
    if(isnan(CAN_ID) || CAN_ID==-1){CAN_ID = 1;}       // CAN bus ID
    if(isnan(CAN_MASTER) || CAN_MASTER==-1){CAN_MASTER = 0;}                 // CAN bus "master" ID
    if(isnan(CAN_TIMEOUT) || CAN_TIMEOUT==-1){CAN_TIMEOUT = 0;}              // CAN bus timeout period
